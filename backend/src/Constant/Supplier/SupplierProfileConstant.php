<?php

namespace App\Constant\Supplier;

final class SupplierProfileConstant
{
    const SUPPLIER_PROFILE_NOT_EXIST = "supplierProfileNotExist";

    const COMPLETE_ACCOUNT_STATUS_PROFILE_CREATED = "profileCreated";

    const COMPLETE_ACCOUNT_STATUS_PROFILE_COMPLETED = "profileCompleted";

    const COMPLETE_ACCOUNT_IS_EMPTY = "complete account status is empty";

    const WRONG_COMPLETE_ACCOUNT_STATUS = "wrong complete account status";

    const ACTIVE_SUPPLIER_PROFILE_STATUS = true;

    const INACTIVE_SUPPLIER_PROFILE_STATUS = false;

    const INACTIVE_SUPPLIER_PROFILE_RESULT = "supplierProfileIsNotActive";
}
