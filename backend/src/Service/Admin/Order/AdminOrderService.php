...
//    }
     ...
     */
    public function createTestingOrderByAdmin(TestingOrderCreateByAdminRequest $request, int $userId): CanCreateOrderResponse|array|string
    {
        $response = [];
        ...
        for ($counter = 0; $counter < $request->getOrdersCount(); $counter++) {
            ...
            $orderCreateByAdminRequest = $this->autoMapping->map(TestingOrderCreateByAdminRequest::class,
                OrderCreateByAdminRequest::class, $request);
            ...
            $orderCreateByAdminRequest->setOrderCost(OrderCostDefaultValueConstant::ORDER_VALUE_CONST);
            $orderCreateByAdminRequest->setNote(OrderNoteDefaultValueConstant::ORDER_NOTE_DEFAULT_VALUE_CONST);
            $orderCreateByAdminRequest->setDestination(OrderClientLocationConstant::ORDER_CLIENT_LOCATION_ARRAY_CONST[array_rand(OrderClientLocationConstant::ORDER_CLIENT_LOCATION_ARRAY_CONST)]);
            $orderCreateByAdminRequest->setRecipientName(OrderRecipientDefaultValueConstant::ORDER_RECIPIENT_NAME_DEFAULT_VALUE_CONST);
            $orderCreateByAdminRequest->setRecipientPhone(OrderRecipientDefaultValueConstant::ORDER_RECIPIENT_PHONE_DEFAULT_VALUE_CONST);
            $orderCreateByAdminRequest->setDetail(OrderNoteDefaultValueConstant::ORDER_NOTE_DEFAULT_VALUE_CONST);
            ...
            $order = $this->adminOrderManager->createOrderByAdmin($orderCreateByAdminRequest);
            ...
            if ($order) {
                ...
                $response[] = $this->autoMapping->map(OrderEntity::class, TestingOrderCreateByAdminResponse::class, $order);

                // 14 create firebase notification to store
                $this->sendFirebaseNotificationAboutOrderStateForUserByAdmin($storeOwnerProfile->getStoreOwnerId(),
                    $order->getId(), $order->getState(), NotificationConstant::STORE);

                // 15 create firebase notification to captains
                $this->sendFirebaseNotificationToAllCaptainsByAdmin($order->getId());
            }
        }
