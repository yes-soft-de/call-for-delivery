...
class OrderDetailsStoreLoaded extends OrderDetailsState {
  ...
  OrderDetailsStoreLoaded(this.screenState, this.storeOwnerInfo)
      : super(screenState) {
    if (storeOwnerInfo.state == OrderStatusEnum.IN_STORE) {
      makeInvoice = true;
    }
  }
  ...
  List<Widget> getStepper(int currentIndex) {
    List<Widget> steps = [
      ...
      CustomStepForStore(
          status: OrderStatusEnum.IN_STORE, currentIndex: currentIndex),
      Expanded(
        child: Padding(
          padding: const EdgeInsets.only(right: 4.0, left: 4.0),
          child: Opacity(
  ...
  void acceptOrder() {
    screenState.requestStoreOrderProgress(
        UpdateStoreOrderStatusRequest(
            ...
            state: StatusHelper.getStatusString(OrderStatusEnum.values[
                StatusHelper.getOrderStatusIndex(storeOwnerInfo.state) + 1])),
        StatusHelper.getOrderStatusIndex(storeOwnerInfo.state));
  }

  Widget _getNextStageCard(BuildContext context) {
    if (storeOwnerInfo.state == OrderStatusEnum.GOT_CAPTAIN) {
      return Padding(
        padding: const EdgeInsets.all(8.0),
        child: InkWell(
          borderRadius: BorderRadius.circular(10),
    ...
    } else if (storeOwnerInfo.state == OrderStatusEnum.IN_STORE) {
      return Padding(
        padding: const EdgeInsets.all(8.0),
        child: InkWell(
          borderRadius: BorderRadius.circular(10),
