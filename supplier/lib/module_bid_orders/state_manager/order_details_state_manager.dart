...
import 'package:c4d/module_bid_orders/ui/state/order_details/ongoing_order_detailes_loaded_state.dart';
...
class OrderDetailsStateManager {
  ...
  void getOnGoingOrderDetails(OrderDetailsScreenState screenState,int orderId ){
    _stateSubject.add(LoadingState(screenState));
    _ordersService.getOrderDetails(orderId).then((value) {
      if(value.hasError){
        _stateSubject.add(ErrorState(screenState,
      ...
      value as OrderDetailsModel;
      _stateSubject
          .add(OngoingOrderDetailsStateLoaded(screenState,value.data));
    });
  }
  ...
  void confirmOffer(OrderDetailsScreenState screenState,ConfirmOfferRequest request ,int orderId){
    _stateSubject.add(LoadingState(screenState));
    _ordersService.confirmOffer(request).then((value) {
      if(value.hasError){
        getOrderDetails(screenState, orderId);
        CustomFlushBarHelper.createError(
            title: S.current.warnning, message: value.error ?? '')
